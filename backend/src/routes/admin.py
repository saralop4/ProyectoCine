...
def register():
    ...
    with conn:
        try:
            ...
            response = canRegisterUser(conn, dataToValidate, 'Administradores')

            userId = setPrefixUser(conn, 'Admin')
            ...
            sql = "INSERT INTO Administradores VALUES (?,?,?,?,?, datetime('now'))"
...
@admin.route('/delete-users', methods=['DELETE'])
def adminLogin():
    conn = create_connection()
    with conn:
        try:
            ...
            sql = "DELETE FROM UsuariosFinal WHERE idUsuariosFinal = ?"
            values = (idUsuario,)
